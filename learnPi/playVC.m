...
- (void)viewDidLoad {
    ...
    self.currentDigit = 0; // do I need this?
    
    [self loadScoreLabels];
    [self loadKeypad];
    [self initializePiLabel];
    
    self.view.backgroundColor = self.colorInst.playBackgroundColor;
...
{
    self.currentDigit++;
    self.currentDigitLabel.text = [NSString stringWithFormat:@"%@%d", @"Current Digit: ", self.currentDigit];
}
...
- (void)loadScoreLabels
{
    ...
    // high score
    self.highScoreLabel = [[UILabel alloc] initWithFrame:CGRectMake(50,120+(self.screenWidth/10-50),200,50)];
    self.highScoreLabel.font = [UIFont fontWithName:self.colorInst.themeFont size:20];
    ...
    // current digit
    self.currentDigitLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.screenWidth-200-50,120+(self.screenWidth/10-50),200,50)];
    self.currentDigitLabel.font = [UIFont fontWithName:self.colorInst.themeFont size:20];
...
{
    NSString *tempStr = [self.piRealString substringWithRange:NSMakeRange(self.currentDigit, 1)];
...
- (void)loadKeypad
{
    ...
    NSLog(@"screen width = %f", self.screenWidth);
    if (self.screenWidth < 568.0) // is iPhone 4 or less
    {
        radius = 30; // test this
    }
    else if (self.screenWidth == 568.0) // is iPhone 5
    {
        radius = 35; // test this
    }
    else if (self.screenWidth == 667.0) // is iPhone 6
    {
        radius = 40;
    }
    else if (self.screenWidth == 736.0) // is iPhone 6+
    {
        radius = 45; // test this
    }
    ...
    CGFloat upperHeight = self.screenHeight*.55;
    CGFloat lowerHeight = self.screenHeight*.75;
    //CGFloat upperSpace = (screenWidth-(5*diameter))/6;
    CGFloat lowerSpace = (self.screenWidth-(6*diameter))/7;
...
- (void) updatePiColors
{
    ...
    int count = 0;
    while(count < (int)[pi2 length])
    {
        NSString *tempStr = [pi2 substringWithRange:NSMakeRange(count, 1)];
...
- (void)recolorViews
{
    ...
    self.zeroView.layer.borderColor = self.colorInst.zeroColor.CGColor;
    self.zeroLabel.textColor = self.colorInst.zeroColor;
    
    self.oneView.backgroundColor = [UIColor clearColor];
    self.oneView.layer.borderColor = self.colorInst.oneColor.CGColor;
    self.oneLabel.textColor = self.colorInst.oneColor;
    
    self.twoView.backgroundColor = [UIColor clearColor];
    self.twoView.layer.borderColor = self.colorInst.twoColor.CGColor;
    self.twoLabel.textColor = self.colorInst.twoColor;
    
    self.threeView.backgroundColor = [UIColor clearColor];
    self.threeView.layer.borderColor = self.colorInst.threeColor.CGColor;
    self.threeLabel.textColor = self.colorInst.threeColor;
    
    self.fourView.backgroundColor = [UIColor clearColor];
    self.fourView.layer.borderColor = self.colorInst.fourColor.CGColor;
    self.fourLabel.textColor = self.colorInst.fourColor;
    
    self.fiveView.backgroundColor = [UIColor clearColor];
    self.fiveView.layer.borderColor = self.colorInst.fiveColor.CGColor;
    self.fiveLabel.textColor = self.colorInst.fiveColor;
    
    self.sixView.backgroundColor = [UIColor clearColor];
    self.sixView.layer.borderColor = self.colorInst.sixColor.CGColor;
    self.sixLabel.textColor = self.colorInst.sixColor;
    
    self.sevenView.backgroundColor = [UIColor clearColor];
    self.sevenView.layer.borderColor = self.colorInst.sevenColor.CGColor;
    self.sevenLabel.textColor = self.colorInst.sevenColor;
    
    self.eightView.backgroundColor = [UIColor clearColor];
    self.eightView.layer.borderColor = self.colorInst.eightColor.CGColor;
    self.eightLabel.textColor = self.colorInst.eightColor;
    
    self.nineView.backgroundColor = [UIColor clearColor];
    self.nineView.layer.borderColor = self.colorInst.nineColor.CGColor;
    self.nineLabel.textColor = self.colorInst.nineColor;
    
    self.dotView.backgroundColor = [UIColor clearColor];
    self.dotView.layer.borderColor = self.colorInst.dotColor.CGColor;
    self.dotLabel.textColor = self.colorInst.dotColor;
...
{
    [self recolorViews];
    self.zeroView.backgroundColor = self.colorInst.zeroColor;
    self.zeroLabel.textColor = self.colorInst.playBackgroundColor;
    self.zeroView.layer.borderColor = self.colorInst.playBackgroundColor.CGColor;
    if ([self isDigitCorrect:@"0"])
    {
        NSLog(@"0 is correct");
        pi2 = [pi2 stringByAppendingString:@"0"];
        [self updatePiLabel];
    }
}
...
{
    [self recolorViews];
    self.oneView.backgroundColor = self.colorInst.oneColor;
    self.oneLabel.textColor = self.colorInst.playBackgroundColor; // use black or white????????
    self.oneView.layer.borderColor = self.colorInst.playBackgroundColor.CGColor;
    if ([self isDigitCorrect:@"1"])
    {
        NSLog(@"1 is correct");
        pi2 = [pi2 stringByAppendingString:@"1"];
        [self updatePiLabel];
    }
}
...
{
    [self recolorViews];
    self.twoView.backgroundColor = self.colorInst.twoColor;
    self.twoLabel.textColor = self.colorInst.playBackgroundColor;
    self.twoView.layer.borderColor = self.colorInst.playBackgroundColor.CGColor;
    if ([self isDigitCorrect:@"2"])
    {
        NSLog(@"2 is correct");
        pi2 = [pi2 stringByAppendingString:@"2"];
        [self updatePiLabel];
    }
}
...
{
    [self recolorViews];
    self.threeView.backgroundColor = self.colorInst.threeColor;
    self.threeLabel.textColor = self.colorInst.playBackgroundColor;
    self.threeView.layer.borderColor = self.colorInst.playBackgroundColor.CGColor;
    if ([self isDigitCorrect:@"3"])
    {
        NSLog(@"3 is correct");
        pi2 = [pi2 stringByAppendingString:@"3"];
        [self updatePiLabel];
    }
}
...
{
    [self recolorViews];
    self.fourView.backgroundColor = self.colorInst.fourColor;
    self.fourLabel.textColor = self.colorInst.playBackgroundColor;
    self.fourView.layer.borderColor = self.colorInst.playBackgroundColor.CGColor;
    if ([self isDigitCorrect:@"4"])
    {
        NSLog(@"4 is correct");
        pi2 = [pi2 stringByAppendingString:@"4"];
        [self updatePiLabel];
    }
}
...
{
    [self recolorViews];
    self.fiveView.backgroundColor = self.colorInst.fiveColor;
    self.fiveLabel.textColor = self.colorInst.playBackgroundColor;
    self.fiveView.layer.borderColor = self.colorInst.playBackgroundColor.CGColor;
    if ([self isDigitCorrect:@"5"])
    {
        NSLog(@"5 is correct");
        pi2 = [pi2 stringByAppendingString:@"5"];
        [self updatePiLabel];
    }
}
...
{
    [self recolorViews];
    self.sixView.backgroundColor = self.colorInst.sixColor;
    self.sixLabel.textColor = self.colorInst.playBackgroundColor;
    self.sixView.layer.borderColor = self.colorInst.playBackgroundColor.CGColor;
    if ([self isDigitCorrect:@"6"])
    {
        NSLog(@"6 is correct");
        pi2 = [pi2 stringByAppendingString:@"6"];
        [self updatePiLabel];
    }
}
...
{
    [self recolorViews];
    self.sevenView.backgroundColor = self.colorInst.sevenColor;
    self.sevenLabel.textColor = self.colorInst.playBackgroundColor;
    self.sevenView.layer.borderColor = self.colorInst.playBackgroundColor.CGColor;
    if ([self isDigitCorrect:@"7"])
    {
        NSLog(@"7 is correct");
        pi2 = [pi2 stringByAppendingString:@"7"];
        [self updatePiLabel];
    }
}
...
{
    [self recolorViews];
    self.eightView.backgroundColor = self.colorInst.eightColor;
    self.eightLabel.textColor = self.colorInst.playBackgroundColor;
    self.eightView.layer.borderColor = self.colorInst.playBackgroundColor.CGColor;
    if ([self isDigitCorrect:@"8"])
    {
        NSLog(@"8 is correct");
        pi2 = [pi2 stringByAppendingString:@"8"];
        [self updatePiLabel];
    }
}
...
{
    [self recolorViews];
    self.nineView.backgroundColor = self.colorInst.nineColor;
    self.nineLabel.textColor = self.colorInst.playBackgroundColor;
    self.nineView.layer.borderColor = self.colorInst.playBackgroundColor.CGColor;
    if ([self isDigitCorrect:@"9"])
    {
        NSLog(@"9 is correct");
        pi2 = [pi2 stringByAppendingString:@"9"];
        [self updatePiLabel];
    }
}
...
{
    [self recolorViews];
    self.dotView.backgroundColor = self.colorInst.dotColor;
    self.dotLabel.textColor = self.colorInst.playBackgroundColor;
    self.dotView.layer.borderColor = self.colorInst.playBackgroundColor.CGColor;
    if ([self isDigitCorrect:@"."])
    {
        NSLog(@". is correct");
        pi2 = [pi2 stringByAppendingString:@"."];
        [self updatePiLabel];
    }
}
